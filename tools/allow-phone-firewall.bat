...
set "JAVA_EXE="
if exist "%LOCALAPPDATA%\TM Companion\java-exe.txt" (
  set /p JAVA_EXE=<"%LOCALAPPDATA%\TM Companion\java-exe.txt"
)
...
if exist "%JAVA_EXE%" (
  netsh advfirewall firewall add rule name="TM Companion Java" dir=in action=allow program="%JAVA_EXE%" protocol=TCP profile=any enable=yes
  netsh advfirewall firewall add rule name="TM Companion Java UDP" dir=in action=allow program="%JAVA_EXE%" protocol=UDP profile=any enable=yes
)
if errorlevel 1 (
...
echo Start TM Companion, then on the phone open the https:// name it prints.
